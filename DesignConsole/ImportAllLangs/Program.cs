...
internal class ImportAllLangs {
  internal static void Run() {
    using (FileStream stream = File.Open(@"c:\rw\libs\DesignConsole\ImportAllLangs\RewiseJazyky.xlsx", FileMode.Open, FileAccess.Read))
    using (IExcelDataReader excelReader = ExcelReaderFactory.CreateOpenXmlReader(stream)) {
      ...
      //to XML
      new Metas() { Langs = metas }.toFile(@"c:\rw\libs\DesignConsole\ImportAllLangs\RewiseJazyky.xml");
    }
  }
}
